...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="bid">Feedback_ID:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO feedback(Feedback_ID, Course_ID, User_ID, Rating) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $bid, $cid, $uid, $rt);
      ...
      include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $bid = $row['Feedback_ID']; // Fetch the Feedback_ID
                echo "<tr>
                    <td>" . $row['Feedback_ID'] . "</td>
                    <td>" . $row['Course_ID'] . "</td>
                    <td>" . $row['User_ID'] . "</td>
                    <td>" . $row['Rating'] . "</td>
                    <td><a style='padding:4px' href='delete_feedback.php?Feedback_ID=$bid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_feedback.php?Feedback_ID=$bid'>Update</a></td> 
                </tr>";
            }
        } else {
